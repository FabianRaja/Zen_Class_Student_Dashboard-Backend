...
export function querySubmission(data){
    return User.findByIdAndUpdate({_id:new ObjectId(data.id)},{$set:{"query.category":data.category,"query.subCategory":data.subCategory,"query.voice":data.voice,"query.queryTitle":data.queryTitle,"query.queryDescription":data.queryDescription,"query.from":data.queryFrom,"query.to":data.queryTo,"query.file":data.file}})
}
